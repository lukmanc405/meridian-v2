...
export async function deployPosition({
  ...
  bins_below,
  bins_above,
  ratio_token, // 0-1 for spot strategy. e.g. 0.75 = 75% token, 25% SOL. Overrides amount_x calculation.
  ...
  // ratio_token support: for spot strategy, compute amount_x from ratio
  // ratio_token = 0.75 means 75% of SOL value goes to token X, 25% to SOL Y
  if (ratio_token != null && ratio_token > 0 && strategy === "spot") {
    const solTotal = amount_sol ?? amount_y ?? 0;
    const solY = solTotal * (1 - ratio_token);
    const solX = solTotal * ratio_token;

    // Get price at active bin to compute token amount
    const pricePerToken = Number(pool.fromPricePerLamport(Number(activeBin.price))); // SOL per token
    const tokenAmount = pricePerToken > 0 ? solX / pricePerToken : 0;

    finalAmountY = solY;
    finalAmountX = tokenAmount;
    log("deploy", `ratio_token=${ratio_token}: split ${solTotal} SOL → ${finalAmountX.toFixed(6)} X @ price ${pricePerToken} SOL/token, ${finalAmountY.toFixed(6)} Y SOL`);
  }
