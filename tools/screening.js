...
export async function getTopCandidates({ limit = 10 } = {}) {
  ...
  const eligible = pools
    .filter((p) => {
      ...
      // Layer 3: Token age > 1 year → skip
      if (p.token_age_hours && p.token_age_hours > 8760) {
  ...
  // enrichPvpRisk not implemented — skip PVP filter block
  // (avoids crash if enrichPvpRisk function is missing)
  ...
  const tokenPairKey = (p) => `${p.base?.mint}-${p.quote?.mint}`;
  const poolsByPair = {};
  for (const p of eligible) {
    const key = tokenPairKey(p);
    if (!poolsByPair[key]) poolsByPair[key] = [];
    poolsByPair[key].push(p);
  }
  for (const poolGroup of Object.values(poolsByPair)) {
    const best = poolGroup.reduce((a, b) => (a.active_tvl > b.active_tvl ? a : b));
    best.is_best_bin_step = true;
  }

  // Pre-compute bins_below for each candidate: round(35 + (volatility/5)*55) clamped [35,90]
  for (const p of eligible) {
    if (p.volatility != null) {
      const raw = Math.round(35 + (p.volatility / 5) * 55);
      p.bins_below = Math.max(35, Math.min(90, raw));
    } else {
...
function condensePool(p) {
  return {
    ...
    volume_5min: round(p.volume_5min),
