...
export async function recordPerformance(perf) {
  ...
  // Fire-and-forget post-close study
  postCloseStudy(perf).catch(err => log("study_error", `Post-close study failed: ${err.message}`));
}

/**
 * Enhanced post-close study — LLM analyzes the trade and generates actionable insights.
 * Compares what happened vs what would have happened with different strategies,
 * then recommends optimal parameters for similar pools.
 */
export async function postCloseStudy(perf) {
  ...
  const studyPrompt = `
POST-CLOSE STRATEGY STUDY

## Closed Position
- Pool: ${perf.pool_name || perf.pool}
- Strategy used: ${perf.strategy}
- Bin step: ${perf.bin_step}
- Volatility: ${perf.volatility}
- Fee/TVL at entry: ${perf.fee_tvl_ratio}%
- Organic score: ${perf.organic_score}
- Initial value: $${perf.initial_value_usd}
- Final value: $${perf.final_value_usd}
- Fees earned: $${perf.fees_earned_usd}
- PnL: ${pnl_pct.toFixed(2)}% (${pnl_usd >= 0 ? "+" : ""}$${pnl_usd.toFixed(2)})
- Range efficiency: ${range_efficiency.toFixed(1)}%
- Time in range: ${perf.minutes_in_range || 0} min / ${perf.minutes_held} min held
- Close reason: ${perf.close_reason || "agent decision"}

## Your Task
Analyze this trade and produce a structured study with:

1. **WHAT HAPPENED** — brief narrative of price action and why the position closed as it did

2. **STRATEGY COMPARISON** — For this specific pool (bin_step=${perf.bin_step}, volatility=${perf.volatility}):
   - If strategy was "spot": would "bid_ask" have performed better or worse? Why?
   - If strategy was "bid_ask": would "spot" have captured more value? Why?
   - Estimate pnl difference if different strategy was used

3. **OPTIMAL PARAMETERS** — For pools with similar characteristics:
   - Best strategy: spot / bid_ask / other
   - Recommended bin_range (bins_below / bins_above ratio)
   - Optimal entry timing triggers (volume threshold, price vs ATH, etc)

4. **KEY LEARNINGS** — 2-3 concise bullet points for future trades

5. **SHOULD-RETRY?** — Yes/No — if yes, under what exact conditions

Respond in this format (no markdown tables):
STUDY RESULT
WHAT: <2 sentences>
COMPARISON: <strategy comparison>
BETTER_STRATEGY: <spot|bid_ask|same>
EST_PNL_DIFF: <estimated pnl % difference if better strategy used>
OPTIMAL_PARAMS: strategy=<>; bins_below=<>; bins_above=<>; entry_trigger=<>
LEARNINGS: <2-3 bullets>
SHOULD_RETRY: <yes|no> — <conditions if yes>
`;

  try {
    const { content } = await agentLoop(studyPrompt, 3, [], "GENERAL", config.llm.generalModel, 800, { requireTool: false });

    if (!content) {
      log("study", "Post-close study returned no content");
      return null;
    }

    // Parse the study result
    const studyResult = parseStudyResult(content, perf);

    // Store study in lessons.json
    const data = load();
    const studyEntry = {
      id: `study_${perf.position}_${Date.now()}`,
      position: perf.position,
      pool: perf.pool,
      pool_name: perf.pool_name,
      strategy_used: perf.strategy,
      ...studyResult,
      performance: {
        pnl_pct: Math.round(pnl_pct * 100) / 100,
        pnl_usd: Math.round(pnl_usd * 100) / 100,
        fees_earned_usd: Math.round((perf.fees_earned_usd || 0) * 100) / 100,
        range_efficiency: Math.round(range_efficiency * 10) / 10,
        minutes_held: perf.minutes_held,
      },
      close_reason: perf.close_reason,
    ...
    data.performance[data.performance.length - 1].study = studyEntry;
    save(data);

    log("study", `Post-close study complete for ${perf.pool_name || perf.pool}: ${studyResult.better_strategy || "same"} would have been better by ~${studyResult.est_pnl_diff || "?"}%`);
    return studyEntry;
  } catch (err) {
    log("study_error", `Post-close study error: ${err.message}`);
    return null;
  }
}

/**
 * Parse LLM study output into structured fields.
 */
function parseStudyResult(content, perf) {
  const result = {};
  ...
  for (const line of lines) {
    if (line.startsWith("WHAT:")) {
      result.what = line.slice(5).trim();
    } else if (line.startsWith("COMPARISON:")) {
      result.comparison = line.slice(12).trim();
    } else if (line.startsWith("BETTER_STRATEGY:")) {
      const val = line.slice(16).trim().toLowerCase();
      result.better_strategy = val.includes("spot") ? "spot" : val.includes("bid") ? "bid_ask" : "same";
    } else if (line.startsWith("EST_PNL_DIFF:")) {
      const match = line.match(/[-+]?\d+\.?\d*/);
      result.est_pnl_diff = match ? parseFloat(match[0]) : null;
    } else if (line.startsWith("OPTIMAL_PARAMS:")) {
      result.optimal_params = line.slice(15).trim();
    } else if (line.startsWith("LEARNINGS:")) {
      result.learnings = [];
      // Collect following lines as bullets until SHOULD_RETRY
      const idx = lines.indexOf(line);
      for (let i = idx + 1; i < lines.length; i++) {
        if (lines[i].startsWith("SHOULD_RETRY")) break;
        const bullet = lines[i].replace(/^[-*•]\s*/, "").trim();
        if (bullet) result.learnings.push(bullet);
      }
    } else if (line.startsWith("SHOULD_RETRY:")) {
      const val = line.slice(14).trim();
      result.should_retry = val.toLowerCase().startsWith("yes");
      result.retry_conditions = val.replace(/^yes\s*[-—:]\s*/i, "").trim();
    }
  }

  // If no better_strategy found, assume same
  if (!result.better_strategy) {
    result.better_strategy = perf.strategy || "same";
  }
