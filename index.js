...
import { evolveThresholds, getPerformanceSummary, getLessonsForPrompt } from "./lessons.js";
  ...
  try {
    ...
    const candidateBlocks = passing.map(({ pool, sw, n, ti, activeBin, mem }, i) => {
      ...
      const block = [
        ...
        `  metrics: bin_step=${pool.bin_step}, fee_pct=${pool.fee_pct}%, fee_tvl=${pool.fee_active_tvl_ratio}, vol=$${pool.volume_window}, vol_5min=$${pool.volume_5min}, tvl=$${pool.active_tvl}, volatility=${pool.volatility}, mcap=$${pool.mcap}, organic=${pool.organic_score}${pool.token_age_hours != null ? `, age=${pool.token_age_hours}h` : ""}`,
...
TOP CANDIDATES (${passing.length} pools):
...
async function telegramHandler(msg) {
  ...
  if (text === "/help") {
    await sendMessage(`📖 *Command Reference*

*Positions*
` + "`" + `/positions` + "`" + ` — List all open positions
` + "`" + `/close <n>` + "`" + ` — Close position #n
` + "`" + `/set <n> <note>` + "`" + ` — Set instruction for position #n

*Screening*
` + "`" + `/candidates` + "`" + ` — Refresh top pool list
` + "`" + `/learn` + "`" + ` — Study top LPers from eligible pools
` + "`" + `/learn <addr>` + "`" + ` — Study LPers from a specific pool

*Info*
` + "`" + `/briefing` + "`" + ` — Morning briefing (24h summary)
` + "`" + `/thresholds` + "`" + ` — View screening thresholds + stats
` + "`" + `/evolve` + "`" + ` — Trigger threshold evolution
` + "`" + `/status` + "`" + ` — Refresh wallet + positions

*Deploy*
` + "`" + `deploy` + "`" + ` — Scan pools & deploy if eligible
` + "`" + `auto` + "`" + ` — Agent picks best pool & deploys

*Natural Language*
Any other message → agent handles it`).catch(() => {});
    return;
  }
...
Focus on: hold duration, entry/exit timing, what win rates look like, whether scalpers or holders dominate.`,
  ...
  (async () => {
    try {
      const startupStep3 = process.env.DRY_RUN !== "false"
